...
export function reviewRequestEmail(d: { name?: string | null; reviewUrl: string }) {
  ...
  const { html, text } = renderEmail({
    ...
    blocks: [
      ...
      `<table class="email-panel" role="presentation" width="100%" cellpadding="0" cellspacing="0" bgcolor="${COLORS.paper}" style="margin:8px 0 4px;border:1px solid ${COLORS.border};border-radius:16px;background-color:${COLORS.paper};"><tr><td class="email-text" style="padding:20px;">${p("<strong>Our thank-you: R50 off your next order.</strong>")}${p("Submit any honest verified review and we’ll email your personal voucher immediately. Every rating qualifies. The voucher is single-use and valid for 90 days.")}</td></tr></table>`,
