...
export function cartRecoveryEmail(d: {
  ...
  const voucherBlock = `<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin:18px 0;border:1px solid ${COLORS.border};border-radius:16px;background:${COLORS.paper};"><tr><td align="center" style="padding:22px;"><span style="font-family:${FONT_STACK};font-size:11px;letter-spacing:1.5px;text-transform:uppercase;color:${COLORS.inkSoft};">R${d.voucher.amount} off this checkout</span><br/><strong style="display:inline-block;margin-top:8px;font-family:${FONT_STACK};font-size:24px;letter-spacing:.08em;color:${COLORS.ink};">${escapeHtml(d.voucher.code)}</strong><br/><span style="display:inline-block;margin-top:8px;font-family:${FONT_STACK};font-size:12px;color:${COLORS.inkSoft};">Valid through ${escapeHtml(expiry)} · use the same email address</span></td></tr></table>`;
  const unsubscribe = `<p style="margin:24px 0 0;padding-top:18px;border-top:1px solid ${COLORS.border};font-family:${FONT_STACK};font-size:12px;line-height:1.6;color:${COLORS.inkSoft};">Sent by Smelt in Cape Town because you asked us to follow up about this checkout. <a href="${escapeHtml(d.unsubscribeUrl)}" style="color:${COLORS.ink};text-decoration:underline;">Stop these emails</a> or contact hello@saunahat.co.za.</p>`;
