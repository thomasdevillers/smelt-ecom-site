...
export default function RestockPanel({ onExpired }: { onExpired: () => void }) {
  ...
  return <section>
    ...
    {data && !data.received && <details className={styles.login}><summary>Receive the October batch</summary><p>Use only when the shipment has physically arrived. This closes pre-orders for the batch and reserves promised hats before making leftovers available.</p><form onSubmit={receive}><label>Green hats received<input type="number" min={0} max={10000} required value={green} onChange={e => setGreen(Number(e.target.value))} /></label><label>Cream hats received<input type="number" min={0} max={10000} required value={cream} onChange={e => setCream(Number(e.target.value))} /></label><button disabled={busy}>Receive batch and allocate pre-orders</button></form></details>}
